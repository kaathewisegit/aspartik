...
fn erf_impl(z: f64, inv: bool) -> f64 {
	...
	let result = if z < 0.5 {
		if z < 1e-10 {
			z * 1.125
				+ z * 0.003379167095512573896158903121545171688
		} else {
			z * 1.125
...
mod tests {
	...
	fn test_erfc_inv() {
		...
		assert_almost_eq!(
			super::erfc_inv(1e-100),
			15.065574702593,
			1e-11
		);
		assert_almost_eq!(
			super::erfc_inv(1e-30),
			8.1486162231699,
			1e-12
		);
		assert_almost_eq!(
			super::erfc_inv(1e-20),
			6.6015806223551,
			1e-13
		);
		assert_almost_eq!(
