...
impl<D: AsMut<[f64]> + AsRef<[f64]>> OrderStatistics<f64> for Data<D> {
	...
	fn median(&mut self) -> f64 {
		...
		if self.len() % 2 != 0 {
			...
			(self.select_inplace(k.saturating_sub(1))
				+ self.select_inplace(k)) / 2.0
		}
	}
	...
	fn ranks(&mut self, tie_breaker: RankTieBreaker) -> Vec<f64> {
		...
		enumerated.sort_by(|(_, el_a), (_, el_b)| {
			el_a.partial_cmp(el_b).unwrap()
		});
		match tie_breaker {
			RankTieBreaker::First => {
				for (i, idx) in enumerated
					.into_iter()
					.map(|(idx, _)| idx)
					.enumerate()
				{
			...
			_ => {
				...
				for (i, (idx, elt)) in
					enumerated.iter().cloned().enumerate()
				{
					if i == 0 {
						prev_idx = idx;
						prev_elt = *elt;
...
mod tests {
	...
	fn test_quantile_short() {
		let data =
			[-1.0, 5.0, 0.0, -3.0, 10.0, -0.5, 4.0, 0.2, 1.0, 6.0];
		let mut data = Data::new(data);
