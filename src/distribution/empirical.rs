...
impl core::fmt::Display for Empirical {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		let mut enumerated_values =
			self.data.iter().flat_map(|(x, &count)| {
				core::iter::repeat(x.get()).take(count as usize)
			});
...
impl ContinuousCDF<f64, f64> for Empirical {
	fn cdf(&self, x: f64) -> f64 {
		...
		let end = Bound::Included(
			NonNan::new(x).expect("x must not be NaN"),
		);

		let sum: u64 =
			self.data.range((start, end)).map(|(_, v)| v).sum();
		sum as f64 / self.sum as f64
	}

	fn sf(&self, x: f64) -> f64 {
		let start = Bound::Excluded(
			NonNan::new(x).expect("x must not be NaN"),
		);
...
mod tests {
	...
	fn test_display() {
		...
		assert_eq!(
			e.to_string(),
			"Empirical([1.000e0, 1.000e0, 2.000e0])"
		);
		e.add(2.0);
		assert_eq!(
			e.to_string(),
