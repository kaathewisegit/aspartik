...
mod tests {
	...
	#[test]
	fn test_min_max() {
		let min = |x: InverseGamma| x.lower();
		let max = |x: InverseGamma| x.upper();
	...
	fn test_ln_pdf() {
		...
		test_absolute(
			0.1,
			0.1,
			0.0628591853882328004197f64.ln(),
			1e-15,
			ln_pdf(1.2),
		);
		test_absolute(
			0.1,
			1.0,
			0.0297426109178248997426f64.ln(),
			1e-15,
			ln_pdf(2.0),
		);
		test_exact(
			1.0,
			0.1,
			0.04157808822362745501024f64.ln(),
			ln_pdf(1.5),
		);
		test_exact(
			1.0,
			1.0,
			0.3018043114632487660842f64.ln(),
			ln_pdf(1.2),
		);
	}
