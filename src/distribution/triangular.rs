...
mod tests {
	...
	fn test_inverse_cdf() {
		let func = |arg: f64| {
			move |x: Triangular| x.inverse_cdf(x.cdf(arg))
		};
	...
	fn test_continuous() {
		test::check_continuous_distribution(
			&create_ok(-5.0, 5.0, 0.0),
			-5.0,
			5.0,
		);
		test::check_continuous_distribution(
			&create_ok(-15.0, -2.0, -3.0),
			-15.0,
			-2.0,
		);
	}
