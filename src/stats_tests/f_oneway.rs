...
impl std::fmt::Display for FOneWayTestError {
	...
	fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
		match self {
			FOneWayTestError::NotEnoughSamples => {
				write!(f, "must be at least two samples")
			}
...
pub fn f_oneway(
	...
	// be > 0
	let f_dist = FisherSnedecor::new(dft, dfe)
		.expect("degrees of freedom should always be >0 ");
...
mod tests {
	...
	fn test_scipy_example() {
		let tillamook = Vec::from([
			0.0571, 0.0813, 0.0831, 0.0976, 0.0817, 0.0859, 0.0735,
			0.0659, 0.0923, 0.0836,
		]);
		let newport = Vec::from([
			0.0873, 0.0662, 0.0672, 0.0819, 0.0749, 0.0649, 0.0835,
			0.0725,
		]);
		let petersburg = Vec::from([
	...
	fn test_nan_in_data_w_emit() {
		...
		let (statistic, pvalue) =
			f_oneway(sample_input, NaNPolicy::Emit).unwrap();
	...
	fn test_group_length_one_ok() {
		...
		let (statistic, pvalue) =
			f_oneway(sample_input, NaNPolicy::Propogate).unwrap();
	...
	fn test_nan_in_data_w_propogate() {
		...
		let group2 =
			Vec::from([0.0873, 0.0662, 0.0672, 0.0819, 0.0749]);
		let sample_input = Vec::from([group1, group2]);
		let (statistic, pvalue) =
			f_oneway(sample_input, NaNPolicy::Propogate).unwrap();
	...
	fn test_bad_data_sample_contains_same_constants() {
		...
		assert_eq!(
			result,
			Err(FOneWayTestError::SampleContainsSameConstants)
		);
