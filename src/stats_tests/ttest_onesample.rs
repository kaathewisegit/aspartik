...
impl std::fmt::Display for TTestOneSampleError {
	...
	fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
		match self {
			TTestOneSampleError::SampleTooSmall => {
				write!(f, "sample must be len > 1")
			}
...
pub fn ttest_onesample(
	...
	let t_dist = StudentsT::new(0.0, 1.0, df)
		.expect("df should always be non NaN and greater than 0");
...
mod tests {
	...
	fn test_jmp_example() {
		...
		let data = Vec::from([
			20.70f64, 27.46f64, 22.15f64, 19.85f64, 21.29f64,
			24.75f64, 20.75f64, 22.91f64, 25.34f64, 20.33f64,
			21.54f64, 21.08f64, 22.14f64, 19.56f64, 21.10f64,
			18.04f64, 24.12f64, 19.95f64, 19.72f64, 18.28f64,
			16.26f64, 17.46f64, 20.53f64, 22.12f64, 25.06f64,
			22.44f64, 19.08f64, 19.88f64, 21.39f64, 22.33f64,
			25.79f64,
		]);
