...
use crate::distribution::{
	Discrete, DiscreteCDF, Hypergeometric, HypergeometricError,
};
...
pub fn fishers_exact_with_odds_ratio(
	...
	let odds_ratio = {
		if table[1] > 0 && table[2] > 0 {
			(table[0] * table[3]) as f64
				/ (table[1] * table[2]) as f64
		} else {
			f64::INFINITY
		}
	};
	...
	let p_value = {
		...
		match alternative {
			...
			Alternative::TwoSided => {
				...
				if (p_exact - p_mode).abs()
					/ p_exact.max(p_mode) <= 1.0 - EPSILON
				{
					return Ok(1.0);
				}
				...
				if table[0] < mode {
					...
					let guess = binary_search(
						n, n1, n2, mode, p_exact,
						EPSILON, true,
					);
					return Ok(p_lower + 1.0
						- dist.cdf(guess - 1));
				}
...
mod tests {
	...
	fn test_fishers_exact_for_trivial() {
		...
		for table in cases.iter() {
			assert_eq!(
				fishers_exact(table, Alternative::Less)
					.unwrap(),
				1.0
			)
		}
