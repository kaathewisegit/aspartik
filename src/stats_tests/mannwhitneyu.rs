...
fn rankdata_mwu<T: PartialOrd>(
	mut y: Vec<T>,
) -> Result<(Vec<f64>, Vec<usize>), MannWhitneyUError> {
	...
	zipped.sort_by(|(_, a), (_, b)| {
		a.partial_cmp(b).expect(
			"NaN should not exist or be filtered out by this point",
		)
	});
	...
	for i in 1..n {
		if y[i] != y[i - 1] {
			...
			let rank = ordinal_rank as f64
				+ (count as f64 - 1.0) / 2.0;
...
fn calc_mwu_asymptotic_pvalue(
	...
	let s: f64 = (n1 * n2 / 12.0
		* ((n + 1.0) - tie_term as f64 / (n * (n - 1.0))))
		.sqrt();
...
mod tests {
	...
	fn test_bad_data_exact_with_ties() {
		...
		assert_eq!(
			result,
			Err(MannWhitneyUError::ExactMethodWithTiesInData)
		);
	}
	#[test]
