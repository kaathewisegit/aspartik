...
pub fn chisquare(
	...
	let f_exp = match f_exp {
		Some(f_to_validate) => {
			...
			// same sum check
			if f_to_validate.iter().sum::<f64>() as usize
				!= total_samples
			{
				return Err(ChiSquareTestError::FExpInvalid);
			}
	...
	let chi_dist = ChiSquared::new(dof as f64)
		.expect("ddof validity should already be checked");
...
mod tests {
	...
	fn test_scipy_example() {
		let (statistic, pvalue) =
			chisquare(&[16, 18, 16, 14, 12, 12], None, None)
				.unwrap();
		...
		let (statistic, pvalue) =
			chisquare(&[16, 18, 16, 14, 12, 12], None, Some(1))
				.unwrap();
		assert!(prec::almost_eq(statistic, 2.0, 1e-1));
	...
	fn test_wiki_example() {
		...
		let (statistic, _) =
			chisquare(&[5, 8, 9, 8, 10, 20], None, None).unwrap();
		assert!(prec::almost_eq(statistic, 13.4, 1e-1));

		let (statistic, _) = chisquare(
			&[5, 8, 9, 8, 10, 20],
			Some(&[10.0; 6]),
			None,
		)
		...
		let (statistic, pvalue) =
			chisquare(&[44, 56], Some(&[50.0, 50.0]), None)
				.unwrap();
	...
	fn test_bad_data_ddof_invalid() {
		let result =
			chisquare(&[16, 18, 16, 14, 12, 12], None, Some(5));
		assert_eq!(result, Err(ChiSquareTestError::DdofInvalid));
		let result =
			chisquare(&[16, 18, 16, 14, 12, 12], None, Some(100));
		assert_eq!(result, Err(ChiSquareTestError::DdofInvalid));
	}
}
